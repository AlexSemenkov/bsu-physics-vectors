package by.bsu.physics.stepura;
...
public class BillionCoordVector extends Vector {
    ...
    public BillionCoordVector() {
        for (int i = 0; i < 1_000_000_000; i++) coordinate[i] = (byte) random.nextInt();
    }

    @Override
    public double getLength() {
        long sumSquaresOfCoordinates = 0;
        for (byte i : coordinate) {
            sumSquaresOfCoordinates += Math.pow(i, 2);
        }
    ...
    public double getScalarProduct(Vector vector) {
        ...
        return IntStream.range(0, 1_000_000_000).parallel().mapToDouble(i -> this.coordinate[i]
                * billionCoordVector.coordinate[i]).sum();
    }
    ...
    public double getCosAngle(Vector that, int numberOfThreads) {
        ...
        Runnable run1 = () -> scal.set(billionCoordVector.getScalarProduct(this));
